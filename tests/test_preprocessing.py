...
def test_preprocess_data_encoding_and_datatype(create_output_dir):
    ...
    assert 'sex' in processed_data.columns, "sex column is missing in the encoded data"
    assert 'smoker' in processed_data.columns, "smoker column is missing in the encoded data"

    # Check the data type of the encoded columns
    assert processed_data['sex'].dtype == 'int64', "sex_male column is not of type int64"
    assert processed_data['smoker'].dtype == 'int64', "smoker_yes column is not of type int64"
    ...
    assert not processed_data[['sex', 'smoker', 'region']].isnull().values.any(), "Encoded columns contain null values"
...
def test_data_upload(create_output_dir):
    ...
    # Ensure the data file is generated by calling preprocess_data
    preprocess_data(bucket_name=bucket_name, file_key=file_key, output_dir='data')

    # Check if the file exists locally before uploading
    assert os.path.exists(file_key), f"{file_key} does not exist."
